...
describe("UI Components", () => {
  ...
  describe("Badge", () => {
    it("renders children with variant classes", () => {
      ...
      expect(screen.getByText("Green badge").className).toContain(
        "bg-green-100",
      );

      rerender(<Badge variant="amber">Amber badge</Badge>);
      expect(screen.getByText("Amber badge").className).toContain(
        "bg-amber-100",
      );
    });
  });
  ...
  describe("ProgressBar", () => {
    it("renders values and handles custom label and value boundaries", () => {
      const { rerender } = render(
        <ProgressBar value={40} max={100} label="Test Progress" showLabel />,
      );
...
describe("Pages and Main Routing Flow", () => {
  ...
  describe("Dashboard Page details", () => {
    it("displays goal progress and handles action clicks", () => {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "Log a new carbon activity" }),
      );
      fireEvent.click(
        screen.getByRole("button", {
          name: "Open AI assistant for personalised tips",
        }),
      );
      fireEvent.click(
        screen.getByRole("button", { name: "View detailed insights" }),
      );
      fireEvent.click(
        screen.getByRole("button", { name: "Browse eco-friendly tips" }),
      );
    });
    ...
    it("displays empty state if logs array is empty and handles button click", () => {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "Log your first activity" }),
      );
    });
  ...
  describe("LogActivity Page form logic and submission", () => {
    it("submits valid entries, triggers success estimates, and deletes logged activity", async () => {
      ...
      const unusualDeleteBtn = screen.getByRole("button", {
        name: "Delete entry: unusual_sub",
      });
      act(() => {
      ...
      const transportBtn = screen.getByRole("button", {
        name: /Select Transport category/i,
      });
      ...
      const submitBtn = screen.getByRole("button", {
        name: "Save this carbon activity",
      });
      act(() => {
  ...
  describe("Tips Page details", () => {
    it("allows category and difficulty filtering, saved pluralization, and empty filters states", () => {
      ...
      const filterBtn = screen.getByRole("button", {
        name: /Filter by Transport/i,
      });
      fireEvent.click(filterBtn);

      // Filter by Medium difficulty button
      const diffBtn = screen.getByRole("button", {
      ...
      const transFilter = screen.getByRole("button", {
        name: /Filter by Transport/i,
      });
      fireEvent.click(transFilter);
      const hardFilter2 = screen.getByRole("button", {
      ...
      const wasteFilter = screen.getByRole("button", {
        name: /Filter by Waste/i,
      });
      fireEvent.click(wasteFilter);
      const hardFilter = screen.getByRole("button", {
        name: /Filter by hard difficulty/i,
      });
  ...
  describe("Assistant Page details", () => {
    it("handles starter query prompts, typing animations, message logs, and clearing chats", async () => {
      ...
      const queryBtn = screen.getByRole("button", {
        name: "Ask: What's my carbon footprint?",
      });
      act(() => {
      ...
      expect(
        screen.getAllByText("What's my carbon footprint?").length,
      ...
      const suggestionBtn = screen.getAllByRole("button", {
        name: /Quick reply: /i,
      })[0];
      act(() => {
  ...
  describe("Profile Page details", () => {
    it("allows editing details, validates inputs, cancels correctly, and clears app data", () => {
      ...
      const clearBtn = screen.getByRole("button", {
        name: "Clear all app data and reset to defaults",
      });
      act(() => {
        fireEvent.click(clearBtn);
      });
    ...
    it("validates profile fields input accurately", () => {
      ...
      expect(screen.getByRole("alert").textContent).toContain(
        "Monthly goal must be at least 10 kg CO₂e.",
      );
