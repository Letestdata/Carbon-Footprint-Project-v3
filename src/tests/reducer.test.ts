...
describe("AppContext reducer", () => {
  ...
  it("handles UPDATE_PROFILE", () => {
    const action: Action = {
      type: "UPDATE_PROFILE",
      payload: { name: "Bob Smith", monthlyBudgetGoal: 150 },
    };
    const nextState = reducer(INITIAL_STATE, action);
  ...
  describe("DELETE_ENTRY", () => {
    it("returns state unchanged if log date does not exist", () => {
      const action: Action = {
        type: "DELETE_ENTRY",
        payload: { date: "2026-06-14", entryId: "e1" },
      };
      const nextState = reducer(INITIAL_STATE, action);
    ...
    it("removes entry from logs and recalculates co2e, removing log if empty", () => {
      ...
      const stateWithEntry = reducer(INITIAL_STATE, {
        type: "ADD_ENTRY",
        payload: entry,
      });
    ...
    it("deletes an entry from a log when multiple logs exist in state", () => {
      ...
      expect(
        nextState.logs.find((l) => l.date === "2026-06-14")?.entries,
      ).toHaveLength(0);
      expect(
        nextState.logs.find((l) => l.date === "2026-06-15")?.entries,
  ...
  describe("CHECK_ACHIEVEMENTS", () => {
    ...
    it("earns achievements when conditions are met", () => {
      ...
      const stateWithLog = reducer(INITIAL_STATE, {
        type: "ADD_ENTRY",
        payload: entry,
      });
      const action: Action = { type: "CHECK_ACHIEVEMENTS" };
    ...
    it("does not duplicate already earned achievements", () => {
      ...
      const stateWithLog = reducer(stateWithEarned, {
        type: "ADD_ENTRY",
        payload: entry,
      });
      const nextState = reducer(stateWithLog, { type: "CHECK_ACHIEVEMENTS" });

      const count = nextState.earnedAchievements.filter(
        (x) => x === "first-log",
      ).length;
      expect(count).toBe(1);
    });
