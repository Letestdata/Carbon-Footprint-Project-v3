...
const INTENT_PATTERNS: Array<{ pattern: RegExp; intent: Intent }> = [
  ...
  {
    pattern: /transport|car|drive|fly|bus|train|commute/i,
    intent: "transport_tips",
  },
  {
    pattern: /energy|electricity|heating|gas|power|solar/i,
    intent: "energy_tips",
  },
  { pattern: /food|eat|diet|meal|beef|vegan|vegetarian/i, intent: "food_tips" },
  {
    pattern: /shop|buy|cloth|fashion|product|purchase/i,
    intent: "shopping_tips",
  },
  { pattern: /waste|recycle|compost|rubbish|trash/i, intent: "waste_tips" },
  {
    pattern: /average|global|world|compare|comparison/i,
    intent: "compare_average",
  },
  {
    pattern: /paris|target|goal|2 ?degrees?|1\.5 ?degrees?/i,
    intent: "paris_target",
  },
  ...
  {
    pattern: /what is co2|carbon dioxide|greenhouse|global warming/i,
    intent: "what_is_co2",
  },
  {
  ...
  {
    pattern: /breakdown|category|categories|split|proportion/i,
  ...
  switch (intent) {
    ...
    case "monthly_summary": {
      ...
      const breakdown = Object.entries(categories)
        ...
        .map(
          ([cat, kg]) =>
            `  • ${cat.charAt(0).toUpperCase() + cat.slice(1)}: ${Number(kg).toFixed(1)} kg`,
        )
        .join("\n");
      ...
      return buildAssistantMessage(
        `📅 **${new Date().toLocaleString("default", { month: "long", year: "numeric" })} Summary:**\n\n` +
          ...
          (breakdown
            ? `**By category:**\n${breakdown}`
            : `No activities logged yet this month.`) +
          `\n\n` +
          ...
          ? `📊 **Your emissions this month by category:**\n\n` +
              (["transport", "energy", "food", "shopping", "waste"] as const)
                .map((cat) => {
                  const catLogs = logs
                    .filter((l) =>
                      l.date.startsWith(new Date().toISOString().slice(0, 7)),
                    )
                    .flatMap((l) => l.entries)
